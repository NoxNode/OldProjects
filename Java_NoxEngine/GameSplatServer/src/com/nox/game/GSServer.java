			...
			if(data.length > 0) {
				if(data[0] == 'E') {
					...
					if(data[1] == 1) {
						...
						ports[nClients] = Server.getPortFromRoutedPacket(packet);
						server.replyToRoutedPacket(packet, new byte[] { 'R', (byte) nClients });
						nClients++;
					}
				}
				else if(data[0] == 'W') {
					...
					for(int j = 0; j < nClients; j++) {
						if(ips[j].equals(ip)) {
							if(ports[j] == port) {
								...
								dataToSend[0] = (byte) j;
								Serializer.addIntToByteBuffer(dataToSend, xCoords[j], 1, 4);
								Serializer.addIntToByteBuffer(dataToSend, yCoords[j], 5, 4);
								for(int k = 0; k < nClients; k++) {
									server.send(dataToSend, packet.getAddress(), ports[k]);
								}
							}
						}
					}
